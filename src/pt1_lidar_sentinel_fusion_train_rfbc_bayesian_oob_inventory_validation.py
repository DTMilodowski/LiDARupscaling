...
agb_mod = io.copy_xarray_template(template)
agb_mod.values[agb_mod.values==-9999]=np.nan
...
def f(params):
    ...
    df = pd.DataFrame({'model':agb_model,'field':agb_field})
    temp1,temp2,r,temp3,temp4 = stats.linregress(df.dropna()['field'],df.dropna()['model'])
...
algorithm = partial(tpe.suggest, n_startup_jobs=spin_up, gamma=0.2, n_EI_candidates=80)
